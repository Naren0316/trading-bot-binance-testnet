...
Keeping validation separate from both the CLI and the API client makes
it independently testable and reusable (e.g. from a future web UI).
"""
...
def build_order_request(
    ...
    """
    Validate all raw CLI inputs and return a clean OrderRequest.
    Raises ValidationError on any invalid field, with a message
    ...
    if order_type == "STOP_LIMIT":
        if price is None:
            raise ValidationError("STOP_LIMIT orders require --price.")
        if stop_price is None:
            raise ValidationError("STOP_LIMIT orders require --stop-price.")
        validated_price = validate_price(price, "price")
        validated_stop_price = validate_price(stop_price, "stop_price")

    if order_type == "MARKET" and price is not None:
        raise ValidationError("MARKET orders do not accept --price.")
